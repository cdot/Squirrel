...
//            "line-height" : 0,
            ...
            function(conflicts) {
                if (conflicts.length > 0) {
                    ...
                    $.each(conflicts, function(i, c) {
                        var e = c.conflict;
                        $("#squeak_dlg").squirrelDialog("squeakAdd", {
                            severity: "warning",
    ...
    function unsaved_changes(max_changes) {
        ...
        if (cloud.status !== S.IS_LOADED) {
            message.unshift(TX.tx("The $1 hoard $2",
                                  cloud.store
                                  ? cloud.store.options().identifier
                                  : TX.tx("Cloud"),
                                  TX.tx(cloud.status)));
        }
        if (client.status !== S.IS_LOADED) {
            message.unshift(TX.tx("The $1 hoard $2",
                                  client.store.options().identifier,
                                  TX.tx(client.status)));
        }
    ...
    function save_hoards() {
        ...
        function finished() {
            ...
            if (client_ok && cloud_ok) {
                if (client.hoard.options.autosave)
                    $("#squeak_dlg").squirrelDialog("close");
                else
                    // Otherwise leave it open
                    $("#squeak_dlg").squirrelDialog("squeakAdd", TX.tx("Save complete"));
        ...
        function update_cloud_store(cloard) {
            cloard.actions = cloard.actions.concat(client.hoard.actions);
            if (cloud.store) {
            ...
            cloud.store.reads(
                ...
                function(data) {
                    ...
                    if (DEBUG) console.debug(
                        this.options().identifier + " is ready");
                    try {
    ...
    function step_5_init_client_hoard() {
        ...
        if (cloud.store && cloud.store.options().needs_path) {
            $("#store_settings_dlg").squirrelDialog(
    ...
    function step_4_load_client_hoard() {
        ...
        function rebuild_hoard() {
            ...
            client.hoard.reconstruct_actions(
                ...
                function() { // on complete
                    ...
                    var as = client.hoard.actions, i, p, $node;
        ...
        client.store.reads(
            ...
            function(data) {
                try {
                    client.hoard = new Hoard(JSON.parse(data));
                    client.status = S.IS_LOADED;
                } catch (e) {
                ...
                if ((client.store
                     && client.store.options().needs_path
                     || cloud.store
                     && cloud.store.options().needs_path)
                    && !client.hoard.options.store_path) {
                    $("#store_settings_dlg").squirrelDialog("open", rebuild_hoard);
                } else {
                    rebuild_hoard();
                }
            },
    ...
    function before_menu_open(e, ui) {
        ...
        $root
            ...
            .contextmenu("showEntry", "insert_copy",
                         !is_leaf && (typeof clipboard !== "undefined"))
...
var   systemPasteContent = 
    e.clipboardData.getData('text/plain');
        ...
        case "insert_copy":
            if (clipboard) {
                try {
                    ...
                    $("#insert_dlg").squirrelDialog("open", {$node: $node, data: data } );
                } catch (e) {
                    if (DEBUG) debugger;
                }
            }
        ...
        case "add_value":
            $("#add_dlg").squirrelDialog("open", {$node: $node, is_value: true});
            break;

        case "add_subtree":
            $("#add_dlg").squirrelDialog("open", {$node: $node, is_value: false});
            break;
    ...
    function init_ui() {
        ...
        $("#extras_button")
            .on($.getTapEvent(), function(/*evt*/) {
                $("#extras_dlg").squirrelDialog("open");
            });
    ...
    S.insert_data = function(path, data) {
        ...
        client.hoard.actions_from_hierarchy(
            ...
            function(act, next) { // listener
                ...
                if (res !== null)
                    $("#squeak_dlg").squirrelDialog("squeakAdd", res.message);
                if (next)
                    next();
            },
            function() { // chain on complete
                Utils.sometime("update_save");
                $("#squeak_dlg").squirrelDialog(
                    "squeakAdd",
