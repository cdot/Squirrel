...
class Hoard {
    ...
    play_action(e) {
        ...
        function conflict(e, mess) {
            let s = global.TX.tx("Cannot ") + {
                A: global.TX.tx("add reminder to"),
                C: global.TX.tx("cancel reminder"),
                D: global.TX.tx("delete"),
                E: global.TX.tx("change value of"),
                M: global.TX.tx("move"),
                N: global.TX.tx("create"),
                R: global.TX.tx("rename"),
                X: global.TX.tx("constrain")
            }[e.type];
        ...
        if (!parent)
            return conflict(e, global.TX.tx("Parent folder not found"));
        ...
        if (e.type === "N") { // New
            if (node)
                return conflict(e, global.TX.tx("It already exists") + " @" +
                                new Date(node.time));
            ...
            if (!node)
                return conflict(e, global.TX.tx("It does not exist"));

            switch (e.type) {
            case "M": // Move to another parent
                // e.data is the path of the new parent
                let new_parent = this._locate_node(e.data);
                if (!new_parent)
                    return conflict(global.TX.tx("New folder '$1' does not exist",
                                          e.data.join("↘")));
            ...
            case "R": // Rename
                if (parent.data[e.data])
                    return conflict(e, " '" + e.data + "': " +
                                    global.TX.tx("It already exists"));
