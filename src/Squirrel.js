...
/*
 * The application startup process proceeds from "_init_application" though
...
if (typeof module !== "undefined") {
    ...
    EncryptedStore = require("../src/EncryptedStore");
    Translator = require("../src/Translator");
    Tree = require("../src/Tree");
}

if (typeof $ === "undefined")
    throw new Error("Squirrel.js requires jQuery");
...
class Squirrel {
    ...
    step_1_init_cloud_store() {
        ...
        let Store = require("../src/" + self.cloudStorageProvider);
        ...
        if (self.useSteganography) {
            ...
            let StegaStore = require("../src/StegaStore");
    ...
    _init_application() {
        ...
        return self.step_1_init_cloud_store()
            ...
            .then(() => {
                let cloud_needs_auth= self.cloud.store
                    && (self.cloud.store.option("needs_user")
                        || self.cloud.store.option("needs_pass"));
                let client_needs_auth = self.client.store
                    && (self.client.store.option("needs_user")
                        || self.client.store.option("needs_pass"));
                if (self.useEncryption && (
                        client_needs_auth || cloud_needs_auth))
    ...
    undo() {
        ...
        // Replay the reverse of console.logthe action
        let res = self.client.hoard.play_action(
            a,
            function (e) {
                // this:Hoard, e:Action
