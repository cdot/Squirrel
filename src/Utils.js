...
class Utils {
  ...
  /**
   * Load an image from a url and return the ImageData therein.
   * Isolates the caller from platform specifics and image type specifics.
   */
  static loadImageData(url) {
    let img, canvas;
      ...
      if (typeof Image === "undefined") {
        ...
        .then(Canvas => {
          return Canvas.loadImage(url.replace(/^file:\/\//, ""))
		      .then(i => img = i)
          .then(() => canvas = Canvas.createCanvas(img.width, img.height));
        });
