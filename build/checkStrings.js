...
function load(file, ext, exclude) {
  ...
  if (Array.isArray(file)) {
    return Promise.all(file.map(f => load(f, ext, exclude)))
  ...
  .then(files => load(files.map(f => path.join(file, f)), ext, exclude))
  .then(files => files.flat())
  .catch(e => []);
}
...
Promise.all([
  ...
  load([`${basePath}/index.html`, `${basePath}/html`],
       /\.html$/)
  .then(files => scan(
    files,
    /data-i18n(?:|-placeholder|-tooltip|-title|-template)=(["'])(.*?)\1/g)),

  load(path.join(basePath, "src"), /\.js$/)
  .then(files => scan(files, /\.i18n\s*\(\s*(["'])(.*?)\1/g))
  .then(files => scan(files, /\/\*i18n\*\/\s*(["'])(.*?)\1/g)),

  // just to get fileContents
  load(path.join(basePath, "test"), /\.ut$/),

  load(path.join(basePath, "i18n"), /\.json$/, /^index\.json$/),
