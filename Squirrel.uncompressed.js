...
// Callback for use when managing hoards; plays an event that is being
...
function play_event(e) {
    ...
    for (var i = 0; i < e.path.length - 1; i++) {
        $parent_ul = $parent_ul.find("li[data-key='" + e.path[i] + "'] > ul");
    }

    if (e.type === "N") {
        add_node($parent_ul, e.path[e.path.length - 1], e.data, e.time);
    } else if (e.type === "R") {
        debugger;
    } else if (e.type === "E") {
        debugger;
    } else if (e.type === "D") {
        $parent_ul.find("li[data-key='" + e.path[i] + "']").remove();
    } else {
        throw "up";
    }
}
