...
(function(g) {
    var util = {
        ...
        findNextPrime: function(n) {
            for (var i = n; true; i += 1) if (util.isPrime(i))
                return i;
        },
    ...
    Cover.prototype = {
           ...
           // Encode length using 10 characters
            ...
            for (var i = 0; i <= message.length; i += 1) {
                ...
                if (curOverlapping > 0 && oldDec) {
                    ...
                    oldMask = Math.pow(2, codeUnitSize)
                        * (1 - Math.pow(2, -curOverlapping));
                    left = (dec & mask) << curOverlapping;
                    right = (oldDec & oldMask) >> codeUnitSize - curOverlapping;
                    modMessage.push(left + right);
                    if (i < message.length) {
                        mask = Math.pow(2, 2 * t - curOverlapping)
                            * (1 - Math.pow(2, -t));
                        for (var j = 1; j < bundlesPerChar; j += 1) {
                            decM = dec & mask;
                            modMessage.push(decM >> (j - 1)
                                            * t + (t - curOverlapping));
                            mask <<= t;
                        }
                        if (overlapping * (i + 1) % t === 0) {
                            mask = Math.pow(2, codeUnitSize)
                                * (1 - Math.pow(2, -t));
                            decM = dec & mask;
                            modMessage.push(decM >> codeUnitSize - t);
                        } else if (overlapping * (i + 1)
                                   % t + (t - curOverlapping) <= t) {
                            decM = dec & mask;
                            modMessage.push(decM >> (bundlesPerChar - 1)
                                            * t + (t - curOverlapping));
                        }
                    }
            ...
            for (offset = 0;
                ...
                for (var i = 0; i < threshold
                     && i + offset < modMessage.length; i += 1) {
                    q = 0;
                    for (var j = offset; j < threshold + offset
                         && j < modMessage.length; j += 1)
                        q += modMessage[j] * Math.pow(args(i), j - offset);
                    qS[i] = 255 - prime + 1 + q % prime;
                }
                for (var i = offset * 4; i < (offset + qS.length) * 4
                     && i < data.length; i += 4)
                    data[i + 3] = qS[i / 4 % threshold];
                subOffset = qS.length;
            }
            for (index = offset + subOffset;
            ...
            for (var i = (index + 1) * 4 + 3; i < data.length; i += 4)
                data[i] = 255;
            imageData.data = data;
        ...
        decode: function(image, options) {
            ...
            codeUnitSize = options.codeUnitSize || config.codeUnitSize,
            prime = util.findNextPrime(Math.pow(2, t)),
            imageData, data, q, args = options.args || config.args,
            modMessage = [],
            messageCompleted = options.messageCompleted
                || config.messageCompleted;
            if (!t || t < 1 && t > 7)
                throw "Error: Parameter t = " + t + " is not valid: 0 < t < 8";
            var shadowCanvas = document.createElement("canvas"),
            shadowCtx = shadowCanvas.getContext("2d");
            shadowCanvas.style.display = "none";
            ...
            imageData = shadowCtx.getImageData(
                0, 0, shadowCanvas.width, shadowCanvas.height);
            data = imageData.data;
            if (threshold === 1) {
                for (var i = 3, done = false;
                     !done && i < data.length && !done; i += 4) {
                ...
                for (var k = 0, done = false; !done; k += 1) {
                    q = [];
                    for (var i = k * threshold * 4 + 3;
                         i < (k + 1) * threshold * 4
                         && i < data.length && !done; i += 4) {
                        done = messageCompleted(data, i, threshold);
                        if (!done) q.push(data[i] - (255 - prime + 1));
                    }
                    if (q.length === 0) continue;
                    var variableCoefficients = function(i) {
                        if (i >= q.length)
                            return [];
                        return [ q[i] * util.product(function(j) {
                            if (j != i) {
                                return util.product(function(l) {
                                    if (l != j)
                                        return args(j) - args(l);
                                }, q.length);
                            }
                        }, q.length) ].concat(arguments.callee(i + 1));
                    }(0);
                    var orderVariableCoefficients = function(order, varIndex) {
                        var workingArgs = util.createArrayFromArgs(
                            args, varIndex, q.length),
                        maxRec = q.length - (order + 1);
                        return function(startIndex, endIndex, recDepth) {
                            var recall = arguments.callee;
                            return util.sum(function(i) {
                                if (recDepth < maxRec)
                                    return workingArgs[i] * recall(
                                        i + 1, startIndex + order + 2,
                                        recDepth + 1);
                            }, endIndex, {
                                start: startIndex,
                                defValue: 1
                    ...
                    for (var i = 0; i < q.length; i += 1) {
                        modMessage.push(
                            (Math.pow(-1, q.length - (i + 1))
                             * util.sum(function(j) {
                                 return orderVariableCoefficients(
                                     i, j) * variableCoefficients[j];
                        }, q.length) % prime + prime) % prime);
                    }
                }
            }
