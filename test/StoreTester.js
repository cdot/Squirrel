...
class StoreTester {
  ...
  configureStore() {
    ...
    if (store.option("needs_url")) {
      for (let key in { net_user: 1, net_pass: 1 }) {
        const v = config[key];
        if (this.debug) this.debug("needs_url:", key, "=", v);
        store.option(key, v);
        four01[key] = v;
      }
    }
    ...
    store.option("network_login", function() {
      ...
      store.option("net_pass", four01.net_pass);
			store.auth = {
        user: store.option("net_user"),
        pass: store.option("net_pass")
      };
  ...
  addTests(suite) {
		...
		suite.addTest(new Test("Write/Read 1 byte", () => {
      const store = this.store;
    ...
    suite.addTest(new Test("Write/Read 0 bytes", () => {
      const store = this.store;
    ...
    suite.addTest(new Test("Write/Read empty string", () => {
      const store = this.store;
    ...
    suite.addTest(new Test("Read non-existant byte data", () => {
      const store = this.store;
    ...
    suite.addTest(new Test("Read non-existant string", () => {
      const store = this.store;
    ...
    suite.addTest(new Test("Write/read binary data", () => {
      const store = this.store;
    ...
    suite.addTest(new Test("Write/Read string", () => {
      const store = this.store;
    ...
    if (this.store.option("needs_url")) {
      ...
      // by us
      suite.addTest(new Test("Handles incorrect net user", () => {
        const store = this.store;
        return store.writes(test_path, TESTR)
        .then(function () {
      ...
      // Won't do anything in the browser, as 401 isn't handled
      // by us
      suite.addTest(new Test("Handles incorrect net pass", () => {
        const store = this.store;
        return store.writes(test_path, TESTR)
        .then(function () {
          // Switch off 401 handler
    ...
    if (this.store.option("needs_user")) {
      suite.addTest(new Test("Handles incorrect user", () => {
        const store = this.store;
    ...
    if (this.store.option("needs_pass")) {
      ...
      suite.addTest(new Test("Handles incorrect password", () => {
        const store = this.store;
  ...
  setup(suite, params) {
    ...
    .then(() => this.configureStore())
    .then(() => this.store.init())
    .then(() => this.addTests(suite));
  }
}
